...
import datetime
# 自定义模块导入
from .models import WorkOrder,OrderStatistics
...
class WorkOrderListView(LoginRequiredMixin, PaginationMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.pk ==1:
            queryset = queryset.filter(status__lt=2).all()
        else:
            queryset = queryset.filter(status__lt=2).filter(assign_to__exact=self.request.user.pk)
...
class WorkOrderDetailView(LoginRequiredMixin,DetailView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if work_order.status == 0:
            work_order.status=1
            work_order.save()
            return render(request,'order/workorder_detail.html',{"work_order":work_order,"msg":"您已接受工单。"})
...
class WorkOrderHistoryView(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.pk ==1:
            ...
            queryset = queryset.filter(status__gt=1).filter(assign_to__exact=self.request.user.pk)
        return  queryset


class WorkOrderDailyView(LoginRequiredMixin,View):
    def get(self,request):
        user_id = request.user.id
        date_start = datetime.date.today() -datetime.timedelta(days=7)
        data_src = OrderStatistics.objects.filter(uid_id__exact=user_id).order_by('statime').filter(statime__gte=date_start)
        res = {
            'title':['数据库','WEB服务','计划任务','配置文件','其他'],
            'date':[res.statime for res in data_src],
            'series':
                [
                {
                    'name':'数据库',
                    ...
                    'barGap': 0,
                    'label': 'labelOption',
                    'data':[res.type0 for res in data_src]
                },
                {
                    'name':'WEB服务',
                    'type':'bar',
                    'barGap': 0,
                    'label': 'labelOption',
                    'data':[res.type1  for res in data_src]
                },
                {
                    'name':'计划任务',
                    'type':'bar',
                    'barGap': 0,
                    'label': 'labelOption',
                    'data':[res.type2 for res in data_src]
                },
                {
                    'name':'配置文件',
                    'type':'bar',
                    'barGap': 0,
                    'label': 'labelOption',
                    'data':[res.type3 for res in data_src]
                },
                {
                    'name':'其他',
                    'type':'bar',
                    'barGap': 0,
                    'label': 'labelOption',
                    'data':[res.type4 for res in data_src]
                },
...
class TestEcharts(TemplateView):
